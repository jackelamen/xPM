        ...
        try {
            ...
            if (projectIds.length) {
                const { data: taskData, error: taskError } = await supabase
                    .from("xpm_tasks")
                    .select("id, project_id, parent_task_id, title, description, status, type, priority, assignee_id, created_by, visibility, private_owner_id, start_date, due_date, due_time, archived_at, created_at, updated_at, custom_fields, milestone, recurrence_rule, recurrence_anchor_date, assignee:profiles!xpm_tasks_assignee_id_fkey(id, name, email, avatar_url)")
    ...
    async ({ workspaceId, projectId, title, description, status, priority, type, leadId, assigneeIds, startDate, dueDate, dueTime, customFields }, { rejectWithValue }) => {
        try {
            ...
            const { data, error } = await supabase
                ...
                .select("id, project_id, title, description, status, type, priority, assignee_id, created_by, visibility, private_owner_id, start_date, due_date, due_time, archived_at, created_at, updated_at, custom_fields, milestone, recurrence_rule, recurrence_anchor_date, assignee:profiles!xpm_tasks_assignee_id_fkey(id, name, email, avatar_url)")
                .single()
