...
const Sidebar = ({ isSidebarOpen, setIsSidebarOpen }) => {
    ...
    const baseClass = 'flex items-center gap-2.5 py-1.5 px-2.5 rounded-md transition-all text-[13px] font-medium mb-0.5'
    ...
    return (
        <div ref={sidebarRef} className={`z-10 bg-white/70 dark:bg-black/40 backdrop-blur-xl min-w-[220px] flex flex-col h-screen border-r border-white/50 dark:border-white/[0.06] max-sm:absolute transition-all ${isSidebarOpen ? 'left-0' : '-left-full'}`}>
            <WorkspaceDropdown />
            <div className='flex-1 overflow-y-scroll no-scrollbar flex flex-col py-2'>
                <div className='px-2'>
                    ...
                    <div className='mb-0.5'>
                        <div className='flex items-center rounded-md overflow-hidden'>
                            <NavLink
                                to='/spaces'
                                className={({ isActive }) => `flex items-center gap-2.5 py-1.5 pl-2.5 pr-1 flex-1 rounded-md transition-all text-[13px] font-medium ${isActive ? activeClass : inactiveClass}`}
                            >
                    ...
                    <div className='mb-0.5'>
                        <div className='flex items-center rounded-md overflow-hidden'>
                            <NavLink
                                ...
                                className={({ isActive }) => `flex items-center gap-2.5 py-1.5 pl-2.5 pr-1 flex-1 rounded-md transition-all text-[13px] font-medium ${isActive ? activeClass : inactiveClass}`}
                            >
                                <FolderOpenIcon size={14} strokeWidth={1.75} />
                                <span>Projects</span>
                            </NavLink>
                            <button
            ...
            <div className='px-2 py-2 border-t border-gray-200/80 dark:border-white/[0.06]'>
